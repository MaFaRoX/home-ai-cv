...
function AppContent() {
  ...
  // Helper function to strip photo from CVData before sending to backend
  const stripPhotoFromCVData = (data: CVData): Omit<CVData, 'personalInfo'> & { personalInfo: Omit<CVData['personalInfo'], 'photo'> } => {
  ...
  // Helper function to merge local photo with CV data from backend
  const mergeLocalPhoto = (cvData: CVData, cvId?: number): CVData => {
    if (typeof window === 'undefined') return cvData;
    
    const photoKey = cvId ? `cv-online-photo-${cvId}` : 'cv-online-guest-photo';
    ...
    // If no local photo, ensure photo field exists (empty string)
    return {
      ...cvData,
      personalInfo: {
        ...cvData.personalInfo,
  ...
  // Helper function to save photo separately in localStorage
  const saveLocalPhoto = (photo: string, cvId?: number): void => {
  ...
  const loadUserCVs = useCallback(async () => {
    ...
    try {
      ...
      // Merge local photos with CVs from backend
      const cvsWithPhotos = cvs.map(cv => ({
        ...cv,
        cvData: mergeLocalPhoto(cv.cvData, cv.id),
      }));
      setUserCVs(cvsWithPhotos);
      
      // If we have CVs and no current CV is set, load the first one
      if (cvsWithPhotos.length > 0) {
        setCurrentCV((prevCV) => {
          if (prevCV) {
            // If we already have a CV, keep it but merge photo
            const existingCV = cvsWithPhotos.find(c => c.id === prevCV.id);
            return existingCV || prevCV;
          }
          // Otherwise, set the first CV
          return cvsWithPhotos[0];
        });
        // Set CV data separately to avoid dependency issues
        setCVData((prevData) => {
          // Only update if we don't have data or if it's different
          if (!prevData && cvsWithPhotos.length > 0) {
            return cvsWithPhotos[0].cvData;
          }
          return prevData;
        });
      }
  ...
  useEffect(() => {
    if (isAuthenticated && accessToken) {
      ...
      if (typeof window !== 'undefined') {
        ...
        localStorage.removeItem('cv-online-guest-photo');
      }
      ...
      if (typeof window !== 'undefined') {
        ...
        if (guestCVData) {
          try {
            ...
            // Merge local photo if available
            const mergedData = mergeLocalPhoto(parsed);
  ...
  const handleFormSubmit = async (data: CVData) => {
    // Save photo locally (separate from CV data)
    const photo = data.personalInfo.photo;
    if (photo) {
      if (currentCV) {
        saveLocalPhoto(photo, currentCV.id);
      } else {
    ...
    if (!isAuthenticated || !accessToken) {
      try {
        if (typeof window !== 'undefined') {
          // Save CV data without photo (photo is stored separately)
          const dataWithoutPhoto = stripPhotoFromCVData(data);
          localStorage.setItem('cv-online-guest-data', JSON.stringify(dataWithoutPhoto));
        }
        setCVData(data); // Keep photo in local state for preview
        setStep("preview");
        setViewMode("create");
    ...
    // Authenticated users: save to backend (without photo)
    try {
      setLoading(true);
      
      // Strip photo before sending to backend
      const dataWithoutPhoto = stripPhotoFromCVData(data);
      
      if (currentCV) {
        // Update existing CV
        const { cv } = await cvApi.updateCV(accessToken, currentCV.id, {
          cvData: dataWithoutPhoto as any, // Backend doesn't expect photo field
          title: data.personalInfo.fullName || "My CV",
        });
        setCurrentCV(cv);
        // Merge local photo back into CV data for display
        const mergedData = mergeLocalPhoto(cv.cvData, cv.id);
        setCVData(mergedData);
        ...
        const { cv } = await cvApi.createCV(
          ...
          dataWithoutPhoto as any, // Backend doesn't expect photo field
          1
        );
        setCurrentCV(cv);
        // Merge local photo back into CV data for display
  ...
  const handleReturnFromDemo = () => {
    // Return from demo mode - preserve existing CV data
    setViewMode("create");
    setStep("form");
    // Don't clear cvData or currentCV - preserve user's work
  };
          ...
          <div className="max-w-7xl mx-auto px-4 py-5">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2 flex-wrap justify-end">
                ...
                {viewMode === "demo" && (
                  <Button onClick={handleReturnFromDemo} className="gap-2 bg-gradient-to-r from-green-600 to-teal-600 hover:from-green-700 hover:to-teal-700 shadow-md hover:shadow-lg transition-all duration-200 transform hover:scale-105">
                    <Edit size={16} />
